...
from timiniprint.raster import DitherMode, PixelFormat
...
def test_rendered_pipeline_is_not_overridden_again(explicit_codec, gray_supported, streaming):
    ...
    common = dict(dither_mode=DitherMode.NONE, trim_side_margins=False,
                  trim_top_bottom_margins=False, feed_padding=0)
    settings = PrintSettings(**common, pixel_format_override=PixelFormat.GRAY4,
                             image_encoding_override=ImageEncoding.V5X_GRAY if explicit_codec else None)
    baseline = PrintSettings(**common, pixel_format_override=PixelFormat.GRAY4 if gray_supported else PixelFormat.BW1)
    ...
    assert settings.pixel_format_override is PixelFormat.GRAY4
